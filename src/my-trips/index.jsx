...
const processedDestinations = (() => {
    ...
    const nameMap = new Map();         // For exact name matches
    const aliasMap = new Map();        // For exact alias matches
    const nameSubstringMap = new Map(); // For partial name matches
    const aliasSubstringMap = new Map(); // For partial alias matches
    ...
    return { 
        ...
        nameSubstringMap, 
        aliasSubstringMap,
        allCountries: destinationsData.countries // Keep reference to all countries
    };
})();
...
const isRTL = language === "he";
    ...
    const getDestinationImage = (locationQuery) => {
        ...
        // This handles cases like "Paris, France" looking for "France"
        ...
        queryParts.forEach(part => {
            if (part.length > 2) { // Ignore short parts
                // Check country name substring matches
                if (processedDestinations.nameMap.has(part)) {
                    const country = processedDestinations.nameMap.get(part);
                    return country.imageUrl; // Direct match with a part
                }
                ...
                if (processedDestinations.nameSubstringMap.has(part)) {
                    ...
                    candidates.forEach(country => {
                        // Give more weight to name matches (e.g. score of 3)
                        const score = 3; 
                ...
                if (processedDestinations.aliasSubstringMap.has(part)) {
                    ...
                    candidates.forEach(country => {
                        // Give less weight to alias matches (e.g. score of 2)
                        const score = 2;
                        if (score > bestMatchScore) {
                            bestMatchScore = score;
                            bestMatch = country;
        ...
        // Check if any country name is contained within the query or vice versa
        ...
        // Strategy 5: Check if any alias is contained within the query or vice versa
    ...
    const fetchTrips = async () => {
        ...
        try {
            ...
            // Generate images for all trips at once
            const images = {};
            for (const trip of tripsData) {
                const destination = trip.tripData?.trip?.destination;
                if (destination) {
                    images[trip.id] = getDestinationImage(destination);
                }
            }
            setTripImages(images);
        } catch (error) {
